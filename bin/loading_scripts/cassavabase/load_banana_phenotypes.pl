...
First column must contain unique identifiers, followed by the following column headers:
    ...
    Pedigree  accession  cycle  plant  BTO:0000010

and columns for the phenotypes in ontology ID format (BTO:0000123).
...
# get location and year from the pre-loaded trials ("project") 
#
...
my $coderef = sub {
	...
	#Pedigree accession cycle plant BTO:0000010
        ...
        my $accession = $spreadsheet->value_at($num, "accession");
	# accessions might have whitespaces 
	$accession =~ s/\s+?//g;
	######
