...
my $individual_sth= $dbh->prepare ("INSERT INTO individual (name, sp_person_id,  population_id) values (?,  (select sp_person_id from sgn_people.sp_person where first_name = 'Roger' and last_name = 'Chetelat'), (select population_id from population where name ilike ?))");
...
eval {
    my $infile=open (my $infile, $ARGV[3]) || die "can't open file";   #./TGRCMutantImagesData.txt
...
#	$line = ~s/\s//g;
	...
	my $locus_symbol= $fields[1];#[3];
	my $allele_symbol =$fields[2];#[4];
	if ($fields[3] =~m /next/) {
	    print "next\n";
	    next;
	}
	
	if ($allele_symbol =~ m/--/) {$allele_symbol = "";}#"1";}
	my $name = uc ($fields[0]);#[5];
	#handle tgrc loci that where merged into one
	if ($locus_symbol =~ m/^me$|^cu$/i) {
	    $allele_symbol = $locus_symbol; 
	    $locus_symbol= "Me";
	}
