...
Usage: perl po_annotations.pl -H dbhost -D dbname -o outfile [-vdnfgxp]
...
The generated file should be submitted to POC (po@plantontology.org) or GOC 
...
my $aspect;
my $namespace;
if ($opt_n eq 'plant_structure' ) {    $aspect = "A"; $namespace='anatomy';}
...
#open (OUT, ">$file") ||die "can't open  file $file for writting.\n" ;
...
sub print_pheno_annotations {
	...
	#print STDERR "looking at anotation $count from SGN individual database\n";
	...
	my $ontology_id= $dbxref->get_db_name(). ":" . $dbxref->get_accession();
	##
	my $ind_s; # a variable for individual synonyms. Currently we don't have any in the database
	
	my $object_type = "germplasm";
	###
	my $common_name = $ind->get_common_name;
	my $species = $species{$common_name} || warn "No species found for common name $common_name ! Check your code. \n";
	...
	$date =~ s/-//g;
	##
	my @ev= $annot->get_object_dbxref_evidence();
	foreach my $dbxref_ev(@ev) {
		...
		$db_reference = 'SGN_ref:861'; 
		# warn "!!!No reference found for annotation $ontology_id individual $object_id ($symbol)\n";
	    }
		...
		foreach my $cd(@cv_dbxrefs) {
			...
			#check for duplicates both in xref and in direct annotations:
			my $ontology_id= $cd->get_db_name(). ":" . $cd->get_accession();
			if ( (!grep /$ontology_id/, @{$xref_annot{$object_id}}) && (!grep /$ontology_id/, @{$p_annot{$object_id}}) ) {  
			    #print STDERR "Found xref term '" . $cd->get_db_name() .":" . $cd->get_accession() ."'\n";
			    $xref_count++;
			    push @{ $xref_annot{$object_id} }, $ontology_id;
			    my $info= "SGN_germplasm\t$object_id\t$symbol\t\t$ontology_id\t$db_reference\t$ev_code\t$ev_with\t$aspect\t$object_name\t$ind_s\t$object_type\t$taxon\t$date\tSGN\n";
    ...
    print STDOUT "Found $count_ev direct and $xref_count xref annotations for SGN individuals, printed into out file $opt_o... Done.\n";
}
