...
we now allow submitters to load new organismprops (such as 'sol100' for organisms to be included in the sequencing project) 

 This db patch will grant web_usr with insert permissions to the organismprop table to allow calling $organism->create_organismprops (L<Bio::Chado::Schema::Organism::Orgnaism> ) from the web site (sol100.pl now allows this, and soon sybmitters will be able to add more props) 
...
my $patch_name = '0003_grant_organismprop_permissions.pl';
my $patch_descr = 'This script grants web_user with insert permissions on cvterm ';
...
GRANT insert,select ON public.cvterm TO web_usr;
GRANT usage,select,update ON public.cvterm_cvterm_id_seq TO web_usr;
